...
async function vrMenuAndMovementChecks(page){
 ...
 assert.ok(result.hovered&&result.cursor&&result.closed);assert.ok(result.forward[0]<0&&Math.abs(result.forward[2])<.0001);assert.ok(result.turned[2]>0&&Math.abs(result.turned[0])<.0001);assert.ok(Math.abs(result.placed[0]-2)<.0001&&Math.abs(result.placed[2]-4)<.0001);
}
