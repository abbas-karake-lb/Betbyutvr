...
function load(i){try{const s=JSON.parse(localStorage.getItem(saveKey(i)));if(validSave(s))return s;}catch{}return household(i);}
...
scene.add(new T.HemisphereLight('#fff4d8','#7e9e8b',2.8));const sun=new T.DirectionalLight('#fff1d7',2.1);sun.position.set(-12,25,10);scene.add(sun);
...
const ground=box(world,0,-.12,0,24,.2,24,'#84a777');ground.userData.ground=true;
...
function furniture(o){const g=new T.Group(),c=item(o.type).color;g.position.set(o.x,0,o.z);g.rotation.y=o.rot*Math.PI/2;g.userData.object=o;
 const b=(x,y,z,w,h,d,col=c)=>box(g,x,y,z,w,h,d,col),cy=(x,y,z,r,h,col=c)=>cyl(g,x,y,z,r,h,col);
 const legs=()=>{for(const x of[-.32,.32])for(const z of[-.32,.32])b(x,.3,z,.08,.6,.08,'#7c6348');};
 switch(o.type){
 case'bed':case'doublebed':{let w=o.type==='bed'?.7:.95;b(0,.22,0,w,.35,.95,'#b69268');b(0,.43,0,w,.15,.93,'#e8e4cf');b(0,.52,.14,w,.06,.62);b(0,.54,-.31,w*.8,.1,.22,'#f4eee0');b(0,.62,-.47,w,.7,.07,'#97724f');break;}
 case'sofa':b(0,.24,0,.94,.3,.75);b(0,.5,.28,.94,.6,.18);for(let x of[-.4,.4])b(x,.42,0,.15,.42,.7);b(0,.43,-.05,.63,.08,.55,'#e3a26b');break;
 case'chair':legs();b(0,.54,0,.55,.09,.55);b(0,.86,.24,.55,.66,.07);break;
 case'table':legs();b(0,.69,0,.94,.1,.94);cy(0,.8,0,.1,.14,'#d9c7a7');ball(g,0,.94,0,.16,'#628c63');break;
 case'desk':legs();b(0,.69,0,.95,.1,.7);b(0,1,.1,.5,.4,.08,'#283d46');b(0,.75,-.2,.4,.03,.18,'#d9d4c2');break;
 case'fridge':b(0,.82,0,.82,1.64,.8);b(0,1,-.415,.025,1.48,.02,'#657d78');b(.29,1.18,-.44,.05,.25,.05,'#567779');break;
 case'stove':b(0,.43,0,.9,.85,.85);b(0,.88,0,.91,.045,.86,'#e4e6db');b(0,.4,-.44,.65,.43,.035,'#24363d');for(let x of[-.23,.23])for(let z of[-.2,.2])cy(x,.92,z,.13,.025,'#344447');break;
 case'counter':b(0,.42,0,.92,.82,.85);b(0,.86,0,.98,.1,.91,'#ece5d3');b(0,.48,-.44,.65,.025,.025,'#9b7e59');break;
 case'dresser':b(0,.5,0,.88,1,.6);for(let y of[.2,.5,.8]){b(0,y,-.315,.78,.25,.035,'#c09764');b(0,y,-.35,.16,.03,.04,'#705941');}break;
 case'toilet':cy(0,.21,0,.23,.4);cy(0,.43,-.12,.29,.12);b(0,.65,.24,.55,.56,.22);cy(0,.501,-.12,.18,.018,'#7b9698');break;
 case'sink':b(0,.35,0,.6,.7,.5,'#bca783');b(0,.77,0,.72,.14,.58);cy(0,.85,0,.21,.02,'#7b9698');b(0,.98,.22,.05,.25,.06,'#628c92');break;
 case'shower':b(0,.08,0,.92,.15,.92,'#e4e5da');b(0,.96,.44,.92,1.8,.06);b(-.44,.96,0,.06,1.8,.88,'#accbca');b(.15,1.57,.3,.05,.6,.06,'#65868a');b(.15,1.88,.14,.23,.04,.26,'#65868a');break;
 case'tv':b(0,.22,0,.9,.4,.55,'#bb9162');b(0,.86,0,.94,.64,.09);b(0,.87,-.055,.8,.51,.015,'#80a6a1');b(0,.52,0,.12,.17,.12);break;
 case'bookcase':b(0,.85,.23,.88,1.7,.08);for(const x of[-.43,.43])b(x,.85,0,.08,1.7,.5);for(const y of[.08,.55,1.06,1.64])b(0,y,0,.92,.06,.5);for(let j=0;j<3;j++)for(let i=0;i<5;i++)b(-.32+i*.15,.28+j*.51,0,.1,.3+(i%2)*.08,.27,['#588a8b','#d69a65','#baa369','#b96d69','#65727c'][i]);break;
 case'plant':cy(0,.16,0,.19,.3,'#c98e64');cy(0,.52,0,.035,.55,'#68815a');ball(g,0,.8,0,.32,c);break;
 case'lamp':cy(0,.04,0,.25,.07,'#7f785d');cy(0,.73,0,.035,1.4,'#8f8566');cy(0,1.43,0,.29,.34);break;
 case'rug':cy(0,.018,0,.48,.025);break;
 case'tree':cy(0,1.05,0,.15,2.1,'#8d7250');ball(g,0,2.5,0,1.1,c);ball(g,.6,2.2,.2,.7,'#81a466');break;
 case'pine':cy(0,.7,0,.12,1.4,'#8d7250');for(let i=0;i<3;i++){const m=new T.Mesh(new T.ConeGeometry(.9-i*.18,1.3,7),mat(c));m.position.y=1.3+i*.5;g.add(m);}break;
 case'flowers':b(0,.07,0,.87,.12,.8,'#637749');for(let i=0;i<5;i++)ball(g,(i%3-1)*.25,.25,Math.floor(i/3)*.3-.2,.14,i%2?c:'#efd191');break;
 case'bench':b(0,.44,0,.96,.12,.55);b(0,.76,.24,.96,.52,.07);for(let x of[-.35,.35])b(x,.2,0,.08,.4,.48,'#68786a');break;
 case'fence':for(let x of[-.4,0,.4])b(x,.45,0,.11,.9,.09);for(let y of[.25,.65])b(0,y,0,1,.08,.07);break;
 }return g;
}
...
function rebuild(){clear(staticWorld);
 for(const [k,type]of Object.entries(state.floors)){const [x,z]=k.split(',').map(Number);const floor=box(staticWorld,x,.006,z,.985,.032,.985,item(type)?.color||'#d2bb93');floor.userData.floor={x,z};if(type==='floor')box(staticWorld,x,.023,z,.012,.003,.98,'#b38154');}
 ...
 for(const o of state.objects)staticWorld.add(furniture(o));
 for(const m of personMeshes.values())peopleWorld.remove(m);personMeshes.clear();
 for(const p of state.people){const g=new T.Group();g.userData.person=p.id;const skin=p.skin||'#d0a078';box(g,0,.98,0,.37,.52,.24,p.color);ball(g,0,1.47,0,.22,skin);ball(g,0,1.6,.03,.195,'#4c4038');const legs=[];for(let side of[-1,1]){legs.push(box(g,side*.1,.42,0,.13,.66,.16,'#455968'));box(g,side*.26,.98,0,.11,.47,.13,skin);box(g,side*.1,.08,-.06,.16,.12,.28,'#4d4b44');}g.userData.legs=legs;peopleWorld.add(g);personMeshes.set(p.id,g);}
 ...
 for(const [material,geometries]of grouped){const merged=mergeGeometries(geometries);if(merged)staticMerged.add(new T.Mesh(merged,material));geometries.forEach(g=>g.dispose());}
 dirty=false;
}
...
function buildAt(x,z,hit){if(!inside(x,z))return;const k=key(x,z),wk=edge(x,z,rotation%2);if(tool==='erase'){
 ...
 else{if(state.objects.some(o=>o.x===x&&o.z===z&&o.type!=='rug')||state.people.some(p=>Math.round(p.x)===x&&Math.round(p.z)===z)){notice('That square is occupied.');return;}checkpoint();state.objects.push({id:Date.now(),type:tool,x,z,rot:rotation});}state.money-=c.price;}
...
function worldHit(){const hits=raycaster.intersectObjects([ground,staticWorld,peopleWorld],true);return hits.find(h=>h.object!==marker&&!marker.children.includes(h.object));}
...
function renderUI(){const p=psel();$('personName').textContent=p.name;$('activity').textContent=p.activity;$('funds').textContent='$'+Math.floor(state.money).toLocaleString();$('clock').textContent=`Day ${state.day} · ${String(Math.floor(state.minutes/60)).padStart(2,'0')}:${String(Math.floor(state.minutes%60)).padStart(2,'0')}`;
...
function addPerson(name,color){if(state.people.length>=6){notice('This household has six people already.');return;}let cell;for(let z=5;z<10&&!cell;z++)for(let x=-2;x<=2;x++)if(!blocked(state,x,z)&&!state.people.some(p=>Math.round(p.x)===x&&Math.round(p.z)===z)){cell={x,z};break;}if(!cell)return;const p=person(name.slice(0,24)||'Alex',cell.x,cell.z,color);state.people.push(p);selected=p.id;dirty=true;renderUI();}
...
function drawPanel(){panelButtons=[];ctx.fillStyle='#183a42';ctx.fillRect(0,0,900,1100);ctx.fillStyle='#e5bf78';ctx.font='bold 34px sans-serif';ctx.fillText('LITTLE LIVES',35,55);ctx.fillStyle='#b7ccc5';ctx.font='22px sans-serif';ctx.fillText(`Day ${state.day}   $${Math.floor(state.money)}   ${mode==='build'?'Building · paused':speed+'× speed'}`,35,95);const p=psel();ctx.fillStyle='#f2f0dd';ctx.font='bold 27px sans-serif';ctx.fillText(p.name+' · '+p.activity,35,144);
 ...
 else if(vrPage==='People'){rows=state.people.map(q=>[q.name,()=>{selected=q.id;renderUI();},selected===q.id]);rows.push(['Go to work',()=>$('work').click()],['Chat',()=>$('chat').click()],['Cancel actions',()=>$('stop').click()],['Add person',()=>{addPerson(['Alex','Jade','Sam','Robin','Kit','Rami'][state.people.length%6],['#749bca','#d78773','#a896c5'][state.people.length%3]);}],['Autonomy: '+(autonomy?'on':'off'),()=>{autonomy=!autonomy;$('autonomy').checked=autonomy;drawPanel();}],['Back',()=>{vrPage='Main';drawPanel();}]);}
